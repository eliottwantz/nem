...
>
	<input
		...
		type="file"
		{name}
		class="dropzone-input {classesInput} cursor-copy"
		on:change
		on:dragenter
		on:dragover
		on:dragleave
	...
	/>
	<!-- Interface -->
	<div class="dropzone-interface {classesInterface} ">
		<div class="dropzone-interface-text">
			...
			<!-- Meta Text -->
			{#if $$slots.meta}<small class="dropzone-meta {slotMeta}"><slot name="meta" /></small
				>{/if}
		</div>
	</div>
</div>
