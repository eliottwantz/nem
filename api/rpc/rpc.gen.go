// nem v0.1.0 9424b17677e5eea944807abbec949f976d5beee3
// --
...
func WebRPCSchemaHash() string {
	return "9424b17677e5eea944807abbec949f976d5beee3"
}
...
type ClassServiceAPI interface {
	...
	ListLearnsOfTeacher(ctx context.Context, teacherId string) ([]*Learn, error)
...
func (s *classServiceAPIServer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case "/rpc/ClassServiceAPI/ListTeachersForLearn":
		s.serveListTeachersForLearn(ctx, w, r)
		return
	case "/rpc/ClassServiceAPI/ListLearnsOfTeacher":
		s.serveListLearnsOfTeacher(ctx, w, r)
		return
...
func (s *classServiceAPIServer) serveListTeachersForLearn(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	header := r.Header.Get("Content-Type")
	i := strings.Index(header, ";")
	if i == -1 {
		i = len(header)
	...
	switch strings.TrimSpace(strings.ToLower(header[:i])) {
	case "application/json":
		s.serveListTeachersForLearnJSON(ctx, w, r)
	default:
		err := ErrorWithCause(ErrWebrpcBadRequest, fmt.Errorf("unexpected Content-Type: %q", r.Header.Get("Content-Type")))
		RespondWithError(w, err)
	}
...
func (s *classServiceAPIServer) serveListTeachersForLearnJSON(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	...
	func() {
		...
		ret0, err = s.ClassServiceAPI.ListTeachersForLearn(ctx, reqContent.Arg0, reqContent.Arg1)
	}()
	respContent := struct {
...
func (s *classServiceAPIServer) serveListLearnsOfTeacher(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	header := r.Header.Get("Content-Type")
	i := strings.Index(header, ";")
	if i == -1 {
		i = len(header)
	...
	switch strings.TrimSpace(strings.ToLower(header[:i])) {
	case "application/json":
		s.serveListLearnsOfTeacherJSON(ctx, w, r)
	default:
		err := ErrorWithCause(ErrWebrpcBadRequest, fmt.Errorf("unexpected Content-Type: %q", r.Header.Get("Content-Type")))
		RespondWithError(w, err)
	}
...
func (s *classServiceAPIServer) serveListLearnsOfTeacherJSON(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	var err error
	ctx = context.WithValue(ctx, MethodNameCtxKey, "ListLearnsOfTeacher")
	reqContent := struct {
	...
	func() {
		...
		ret0, err = s.ClassServiceAPI.ListLearnsOfTeacher(ctx, reqContent.Arg0)
	}()
	respContent := struct {
		Ret0 []*Learn `json:"learns"`
	}{ret0}
